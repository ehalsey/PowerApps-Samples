...
namespace BasicOperations
{
    internal class Program
    {
        static async Task Main()
        {
            ...
            var conString = "AuthType=OAuth;Url=https://myorg.crm.dynamics.com;RedirectUri=http://localhost;AppId=51f81489-12ee-4a9e-aaae-a2591f45987d;LoginPrompt=Auto";


            // read the secret from the DATAVERSE_CLIENT_SECRET environment variable
            string clientSecret = Environment.GetEnvironmentVariable("DATAVERSE_CLIENT_SECRET");
            if (string.IsNullOrEmpty(clientSecret))
            {
                throw new Exception("The DATAVERSE_CLIENT_SECRET environment variable is not set.");
            }
            
            //ServiceClient(Uri, String, SecureString, Boolean, ILogger)
            ServiceClient serviceClient = new(
                new Uri("https://pas-poc-dev.crm.dynamics.com"),
                "ac51513c-5916-4039-b4c8-1aa2685637bb",
                clientSecret,
                true);

            // Solution and language information
            string solutionUniqueName = "ACTOPASPOC";
            ...
            string referencingTableSchemaName = "acto_SampleTable";
            ...
            serviceClient.ExecuteAsync(request).Wait();

            return;

        //     List<Microsoft.Xrm.Sdk.EntityReference> recordsToDelete = new();
